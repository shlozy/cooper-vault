...
COLOR_ACCENT = "#10B981"    # Emerald — the app's one primary accent (was royal blue)
...
# `conn` / `cursor` always point at the CURRENTLY LOGGED-IN user's
# private financial database. They're unset until someone logs in or
# registers (see build_login_screen / build_signup_screen / the
# Remember-Me path in show_auth_gate, which all call get_user_db(...)
# and rebind these two names) — every other add/edit/delete/report
# function in the app already just calls bare `cursor.execute(...)` /
# `conn.commit()`, so nothing else needs to change.
conn = None
cursor = None
